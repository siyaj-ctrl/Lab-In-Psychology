...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\lab in psychology\\stroop_task_practice\\Stroop_Task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text='welcome \n\npress r when you see red font colour \nand press b when you see blue font colour\n\npress space to start the experiment\n\n',
        font='Arial',
    ...
    cc = visual.TextStim(win=win, name='cc',
        text='',
        font='Arial',
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('Stroop_practice.xlsx'), 
        seed=None, 
        isTrials=True, 
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[Fixation, cc, key_resp],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        cc.setColor(colour, colorSpace='rgb')
        cc.setText(word)
        ...
        while continueRoutine:
            ...
            # if cc is starting this frame...
            if cc.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                cc.frameNStart = frameN  # exact frame index
                cc.tStart = t  # local t and not account for scr refresh
                cc.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(cc, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'cc.started')
                # update status
                cc.status = STARTED
                cc.setAutoDraw(True)
            
            # if cc is active this frame...
            if cc.status == STARTED:
                # update params
                pass
            
            # if cc is stopping this frame...
            if cc.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > cc.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    cc.tStop = t  # not accounting for scr refresh
                    cc.tStopRefresh = tThisFlipGlobal  # on global time
                    cc.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'cc.stopped')
                    # update status
                    cc.status = FINISHED
                    cc.setAutoDraw(False)
