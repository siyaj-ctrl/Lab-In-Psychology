...
expName = 'Lexixal_Decision_Task'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\lab in psychology\\Solo_Project\\Lexixal_Decision_Task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text='Welcome to the experiment!\n\nYou will see TWO letter strings at a time (e.g., BREAD BUTTER).\n\nPress Y if BOTH are real, related English words.\nPress N if not (if one or both are not real words)\nPlease respond as quickly and accurately as possible.\nPress SPACE to begin.',
        font='Arial',
    ...
    thisExp.currentRoutine = Introduction
    ...
    while continueRoutine:
        ...
        if Introduction.forceEnded or routineForceEnded:
            break
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('cds_words.csv'), 
        seed=None, 
        isTrials=True, 
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[fixation, text, text_4, key_resp],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        text.setText(TOP_STRING)
        text_4.setText(BOTTOM_STRING)
