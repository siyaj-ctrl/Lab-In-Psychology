...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\lab in psychology\\Motor_Sequence\\Motor_Sequence_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "Random" ---
    Fixation_2 = visual.ShapeStim(
        win=win, name='Fixation_2', vertices='cross',units='pix', 
        size=(10, 10),
        ori=0.0, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor=(0.6471, -0.1765, -0.7647),
        opacity=None, depth=0.0, interpolate=True)
    Line_1 = visual.Rect(
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if Trail.forceEnded or routineForceEnded:
                break
    ...
    for thisTrial_3 in trials_3:
        ...
        # --- Prepare to start Routine "Random" ---
        # create an object to store info about Routine Random
        Random = data.Routine(
            name='Random',
            components=[Fixation_2, Line_1, Line_2, Line_3, Line_4, probe, Key_resp],
        )
        Random.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        probe.setPos((pos, 60))
        # create starting attributes for Key_resp
        ...
        Key_resp.rt = []
        _Key_resp_allKeys = []
        # store start times for Random
        Random.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        Random.tStart = globalClock.getTime(format='float')
        Random.status = STARTED
        thisExp.addData('Random.started', Random.tStart)
        Random.maxDuration = None
        # keep track of which components have finished
        RandomComponents = Random.components
        for thisComponent in Random.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Random" ---
        thisExp.currentRoutine = Random
        Random.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # if trial has changed, end Routine now
            ...
            # has a Component requested the Routine to end?
            if not continueRoutine:
                Random.forceEnded = routineForceEnded = True
            # has the Routine been forcibly ended?
            if Random.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in Random.components:
        ...
        # --- Ending Routine "Random" ---
        for thisComponent in Random.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for Random
        Random.tStop = globalClock.getTime(format='float')
        Random.tStopRefresh = tThisFlipGlobal
        thisExp.addData('Random.stopped', Random.tStop)
        # check responses
        if Key_resp.keys in ['', [], None]:  # No response was made
            Key_resp.keys = None
            # was no response the correct answer?!
        ...
        # the Routine "Random" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
