...
expName = 'Staircase_1'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\lab in psychology\\Staircase\\Staircase_1_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Fixati = visual.ShapeStim(
        win=win, name='Fixati', vertices='cross',
        size=(0.1,0.1),
        ori=None, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=1.0,
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[Fixati, grating, key_resp],
        )
        ...
        while continueRoutine:
            ...
            # if Fixati is starting this frame...
            if Fixati.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                Fixati.frameNStart = frameN  # exact frame index
                Fixati.tStart = t  # local t and not account for scr refresh
                Fixati.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(Fixati, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Fixati.started')
                # update status
                Fixati.status = STARTED
                Fixati.setAutoDraw(True)
            
            # if Fixati is active this frame...
            if Fixati.status == STARTED:
                # update params
                pass
            
            # if Fixati is stopping this frame...
            if Fixati.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Fixati.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
                    Fixati.tStop = t  # not accounting for scr refresh
                    Fixati.tStopRefresh = tThisFlipGlobal  # on global time
                    Fixati.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Fixati.stopped')
                    # update status
                    Fixati.status = FINISHED
                    Fixati.setAutoDraw(False)
